...
# (..., n) complex numbers => (..., n, 2) @James Cheng
def cpx2xy(_cpx): 
    _r = _cpx.real.unsqueeze(-1)
    _i = _cpx.imag.unsqueeze(-1)
    _xy = torch.cat((_r, _i), dim=-1)
    return _xy

# (..., n, 2) => (..., n) complex numbers @James Cheng
def xy2cpx(_xy):
    return torch.complex(_xy[..., 0], _xy[..., 1])

def fft1(x):
    """ 1-dimensional centered Fast Fourier Transform. """
    assert x.size(-1) == 2
    x = ifftshift(x, dim=(-2,))
    x = xy2cpx(x) # (..., n, 2) => (..., n) @James Cheng
    x = torch.fft.fft(x, dim=-1, norm ='ortho')
    x = cpx2xy(x) # (..., n) => (..., n, 2) @James Cheng
    x = fftshift(x, dim=(-2,)) 
    return x
...
def ifftshift(x, dim=None):
    ...
    return roll(x, shift, dim)
'''
class EnhancedAlignedLoss(nn.Module):
    def __init__(self):
        super(GuidedDirectionalLoss, self).__init__()
    def forward(self, pred, target):
        MSE = torch.mean((target - pred) ** 2)
        _ += (target - pred)[4*i,:] for i in range(128)
        GD = torch.mean(_ ** 2)
        return MSE + GD
'''        
...
class FanbeamRadon(torch.nn.Module, LinearOperator):
    """ Parametrized implementation of a discrete  fanbeam Radon transform. 参数化实现

    Allows for automatic differentiation with respect to the parameters
    defining the fanbeam geometry.允许对定义扇形梁几何的参数进行自动区分。

    Parameters
    ----------
    n : (int, int)
        离散图像信号的尺寸。目前只在方形图像上测试，所以n=(n1, n2)应该满足n1=n2。
    angles : torch.Tensor
        范围为[0...360]的扇形光束旋转角度阵列
    d_source : torch.Tensor
        X射线源到原点（旋转中心）的距离。
        原点到探测器阵列的距离是由视场角推断出来的，视场角的选择是为了使图像信号中的最大
        刻圆正好位于每个扇形内。（缺失信息大概率问题出在这儿）
    n_detect : int
        探测器阵列中的探测器数量。
    s_detect : torch.Tensor
        沿着检测器阵列的检测器间距。
    flat : bool
        Use a flat or curved detector array. (Default True)使用平坦或弯曲的探测器阵列。(默认为真)
    filter_type : str
        用于滤波反投影（反转）的滤波器。
        是["hamming", "cosine", "hann", "ramp"]之一。(默认为 "hamming")
    learn_inv_scale : bool
        使过滤后的反投影（反演）的缩放因子可以学习。(默认为假)
    """
